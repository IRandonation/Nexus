...
- [ ] Widget 拖拽功能（仅有 CSS cursor-grab，无鼠标事件处理）
- [ ] Insight/Scheduler 服务连接（代码存在但未在 main.rs 注册）
- [ ] 测试套件（当前不存在测试文件）
...
#### P0 - 可用性阻塞（立即）
- [ ] **Widget 拖拽实现**：添加鼠标事件处理 + 窗口定位逻辑，让用户能自由移动浮窗定位
- [ ] **Insight/Scheduler 服务接线**：在 main.rs 注册 InsightService 和 SchedulerService，使已编写代码生效

#### P1 - 短期优化（本周）
- [ ] **日志治理**：完善后端日志记录，标记关键操作节点，便于问题排查
- [ ] **错误边界**：添加前端 React 错误边界组件，防止单点崩溃影响整体
- [ ] **Widget 位置配置化**：将窗口位置持久化到本地存储，重启后保持定位

#### P2 - 中期增强（本月）
- [ ] **测试补齐**：补充关键模块的单元测试和集成测试，保障重构安全
- [ ] **FTS5 全文搜索实现**：启用 SQLite FTS5 扩展，支持任务内容快速检索
...
| 优化项 | 优先级 | 证据来源 | 状态 |
|--------|--------|----------|------|
| Widget 拖拽功能实现 | 高 | L168 下一步计划 P0 一致 | 待处理 |
| Widget 渲染优化（React.memo） | 中 | T1 基线检查发现重复渲染 | 待处理 |
| 数据库查询优化（索引/预编译） | 中 | T1 基线检查发现查询延迟 | 待处理 |

### 代码质量

| 优化项 | 优先级 | 证据来源 | 状态 |
|--------|--------|----------|------|
| 日志治理（println! → tracing） | 高 | T1 基线检查发现控制台调试代码 | 待处理 |
| 错误边界添加 | 中 | T1 基线检查发现错误处理不完善 | 待处理 |

### 技术债务

| 优化项 | 优先级 | 证据来源 | 状态 |
|--------|--------|----------|------|
| Widget 位置硬编码 → 配置化 | 高 | T1 基线检查发现位置写死 | 待处理 |
| 服务注册接线（Insight/Scheduler） | 高 | L169 下一步计划 P0 一致 | 待处理 |
| 测试补齐 | 中 | T1 基线检查发现测试覆盖不足 | 待处理 |
| FTS5 全文搜索实现 | 中 | L178 下一步计划 P2 一致 | 待处理 |
